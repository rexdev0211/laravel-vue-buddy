    ...
    export default {
        ...
        methods: {
            goBackToChat() {
                if (window.history.length > 1) {
                    ...
                    this.goBack(`/chat/event-user/${this.eventId}/${userToken}`);
                }
            },
            loadSwiperScript() {
        ...
        computed: {
            ...
            ...mapState({
                videos(state) {
                    if (this.event) {
                        let interlocutorId = this.isMyEvent(this.event) ? this.userId : this.event.user.id

                        const videos = state.chatModule.chat.event.videos[`${this.eventId}-${interlocutorId}`]
                        if (videos === undefined) {
                            axios.get(`/api/getEventChatVideosList/${this.eventId}/${interlocutorId}`)
                                .then(({data}) => {
                                    const payload = {
