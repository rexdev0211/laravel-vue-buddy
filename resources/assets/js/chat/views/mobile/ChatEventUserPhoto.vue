        ...
        computed: {
            ...
            ...mapState({
                allPhotos: function(state) {
                    if (this.event) {
                        let interlocutorId = this.isMyEvent(this.event) ?
                            this.userId
                            :
                            this.event.user.id;

                        const images = state.chatModule.chat.event.images[`${this.eventId}-${interlocutorId}`]
                        if (images === undefined) {
                            axios.get(`/api/getEventChatImagesList/${this.eventId}/${interlocutorId}`)
                                .then(({data}) => {
                                    const payload = {
