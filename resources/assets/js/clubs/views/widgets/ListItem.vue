<template>
    <div class="event"
        ...
        <div class="img" :style="{'background': `url(${club.photo_orig}) no-repeat center / cover`}">
        ...
        <div class="details">
            ...
            <div class="small-details">
                ...
                <svg style="margin-left:20px;" v-if="club.is_private || invited || accepted" width="20" height="27" viewBox="0 0 20 27" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- <button
          ...
          @click.stop="updateMembership({ eventId: club.id })"
      ...
      <button
          ...
          type="button"
          class="btn decline darker notranslate"
          @click.stop="declineInvite({ eventId: club.id })">
        <img src="/main/img/icons/close-grey.svg" />
      </button>
      <button
          v-if="invited && !accepted"
          type="button"
          class="btn accept notranslate"
          :class="membershipActionBtnClass"
          @click.stop="acceptInvite({ eventId: club.id })">
        <img src="/main/img/icons/check-green.svg" />
      </button>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        props: ['club', 'invited', 'accepted'],
        created() {
          Vue.set(this.$store.state.clubsInfo, this.club.id, this.club)
        },
        computed: {
            members() {
                console.log('club', this.club);
                return this.club.members.filter(member => !member.blocked);
            },
            membersCount() {
                return this.isMobile ? this.members.length - 2 : this.members.length - 3;
            },
            clubInfo() {
              return this.$store.state.clubsInfo[this.club.id] ?? this.club;
            }
        },
        methods: {
            declineInvite(data) {
